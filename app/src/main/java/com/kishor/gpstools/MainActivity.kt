...
class MainActivity : AppCompatActivity() {
    ...
    private fun initializeViews() {
        ...
        startButton = findViewById(R.id.startDatabaseButton)
        stopButton = findViewById(R.id.stopDatabaseButton)
    }
    ...
    private fun startDatabaseBuild() {
        ...
        lifecycleScope.launch {
            ...
            // Show AutoTag prompt ONLY for Move/Copy (not Log)
            if (!viewModel.isCancelled && selectedAction != FileAction.NOTHING) {
                showAutoTagConfirmationDialog()
            }
        }
    }
    
    private fun stopDatabaseBuild() {
        viewModel.cancelOperation()
        startButton.isEnabled = true
        stopButton.isEnabled = false
        Toast.makeText(this, "Process stopped. Check database for progress.", Toast.LENGTH_LONG).show()
    }
